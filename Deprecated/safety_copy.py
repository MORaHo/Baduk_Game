from variables import SIZE, run, height, width, beige, LINE_COLOUR, space, COLOUR_ARRAY, PLAYER_ARRAY, current_player, block, liberties, opposing_player, seki_counter
...
# find position of stone on ui board
def ui_piece_position(position):
    return ((position - space/2) // space ) * space + space # we compensate for the offset ->  integer position (//) -> go to the intersection
...
def determine_if_neighbour(cell_row,cell_column):
    ...
    if last_move_row == cell_row and last_move_column == cell_column -1:
        if_neighbour = True
    if last_move_row == cell_row and last_move_column == cell_column +1:
        if_neighbour = True
    if last_move_row == cell_row-1 and last_move_column == cell_column+1:
        if_neighbour = True
    if last_move_row == cell_row+1 and last_move_column == cell_column:
        if_neighbour = True
...
while run:
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.MOUSEBUTTONDOWN: # If mouse is pressed
            ...
            ui_y_position_of_target_cell = ui_piece_position(mouse_position[0]) # tentative y position for stone, consider deleting
            ui_x_position_of_target_cell = ui_piece_position(mouse_position[1]) # tentative x position for stone, consider deleting
            ...
            if board[cell_row][cell_column] != EMPTY or board[cell_row][cell_row] == OFFBOARD:
                ...
                #place_stone(board_row_of_cell,board_column_of_cell) #placing stone
                
            else:

                fallback_board = numpy.copy(board)
                place_stone(cell_row,cell_column)

                if seki_counter == 1:
                    
                    captures(opposing_player)
                    posticipate_board = numpy.copy(board)
                    #print(cell_row,cell_column)
                    print(moves[opposing_player][-1])
                    #print(not(determine_if_neighbour(cell_row,cell_column)))
