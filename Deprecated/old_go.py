...
width = size * alt + alt
height = size * alt + alt + pass_space
pre_board = None
post_board = None
...
def draw_board() -> None:
    ...
    for i in range(1,size+1):
        ...
        #vertical lines
        pg.draw.line(screen,line_black,(alt,i*alt),(height-alt-pass_space,i*alt),1)
...
def clear_pre_pre_board():
    for y in range(len(board)):
        for x in range(len(board)):
            if pre_pre_board[y][x] != OFFBOARD:
                val = pre_pre_board[y][x]
                new_val = val & 3
                pre_pre_board[y][x] = new_val

def captures(color,pg_color):

    global check
    global seki_count
    global pre_pre_board
    check = False
            ...
            if piece & color:
                ...
                if len(liberties) == 0: 
                    ...
                    if seki_count == 0:
                        ...
                        check = True
                        pre_pre_board = numpy.copy(board)
                        clear_block()
                        seki_count = 1
                        continue
...
while run:
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.MOUSEBUTTONDOWN:
            ...
            if pos_y > (height-pass_space):
                passes += 1
                if passes == 2:
                    run = False
                #Add a step to change colours since currently it is only dictated by a since player
                continue ##this should be changed with logic that check if the game has been passed twice or not and end the same if it has
            ...
            if piece_board[int((pos_y//alt)-1),int((pos_x//alt)-1)] != None:
                ...
                if seki_count == 1:
                    ...
                    clear_pre_pre_board()
                    if (board == pre_pre_board).all() == False:
                        board = pre_board
                        set_stone(int((pos_y//alt)),int((pos_x//alt)),color)
                        captures(3-color,pg_color)
                        if pg_color == black: color = WHITE; pg_color = white
                        else: pg_color = black; color = BLACK
                        draw_board()
                        ### add logic to check if it is seki by checking if placing allows the opponent to capture, if yes => seki = 1, else seki =0
                        passes = 0
                        #continue
                    #print("or here")
                ...
                #any move that doesn't fall within the rules for a ko fight
                    ...
                    # stop player from suiciding if they are not doing seki
                    ...
                    #checking isf there is a capture due to the move, if so redraw the board (cannot just delete since they are drawn, so you have to redraw)
